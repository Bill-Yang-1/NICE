...
class TrainerFullHeadAutoDecoder(object):

    def __init__(self, decoder, cfg, device, train_dataset, val_dataset, exp_name):
    ...
    def train_step(self, batch, epoch):
        ...
        loss_dict_nice, _ = compute_all_loss(batch, self.decoder, self.latent_codes, self.device)

        

        loss_tot = 0
        for key in loss_dict_nice.keys():
            loss_tot += self.cfg['lambdas'][key] * loss_dict_nice[key]
        ...
        loss_dict = {k: loss_dict_nice[k].item() for k in loss_dict_nice.keys()}
    ...
    def compute_val_loss(self, epoch):
        ...
        for val_batch in self.val_data_loader:
            ...
            l_dict, _= compute_all_loss(val_batch, self.decoder, self.latent_codes_val, self.device)
    ...
    def fit_val_dataset(self, epoch):
        ...
        for idx in range(len(self.val_dataset)):
            ...
            trim_val_sft.export(exp_dir + f'test_{iden_val}_sft.ply')
            ...
            trim_val_mxl.export(exp_dir + f'test_{iden_val}_mxl.ply')
            ...
            trim_val_mdb.export(exp_dir + f'test_{iden_val}_mdb.ply')
