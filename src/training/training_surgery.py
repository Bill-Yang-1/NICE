...
from src.models.loss_functions import compute_all_loss_corresp_forward
...
class SurgeryTrainerAutoDecoder(object):
    ...
    def __init__(self,
        ...
        self.latent_codes_shape = torch.nn.Embedding(len(train_dataset)*2, decoder_shape.lat_dim,
        ...
        self.latent_codes = torch.nn.Embedding(len(train_dataset), self.decoder.lat_dim_surgery,
                                               max_norm=1.0, sparse=True, device=device).float()
    ...
    def train_step(self, batch, epoch):
        ...
        loss_dict = compute_all_loss_corresp_forward(batch, self.decoder, self.decoder_shape, self.latent_codes, self.latent_codes_shape, self.device, epoch, self.exp_path)
    ...
    def train_model(self, epochs):
        ...
        for epoch in range(start, epochs):
            ...
            current_encoder_lr = self.optimizer_encoder.param_groups[0]['lr']
            current_lat_lr = self.optimizer_lat.param_groups[0]['lr']
    ...
    def save_best_checkpoint(self, epoch):
        path = self.checkpoint_path + 'Best_checkpoint.tar'.format(epoch)
        torch.save({'epoch': epoch,
                    'decoder_state_dict': self.decoder.state_dict(),
                    'optimizer_encoder_state_dict': self.optimizer_encoder.state_dict(),
                    'optimizer_lat_state_dict': self.optimizer_lat.state_dict(),
    ...
    def compute_val_loss(self, epoch):
        ...
        for val_batch in self.val_data_loader:
            ...
            l_dict = compute_all_loss_corresp_forward(val_batch, self.decoder, self.decoder_shape, self.latent_codes_val,
    ...
    def construct_rec(self, encoding_expr, encoding_shape, mesh, epoch=-1, landmarks=None):
        ...
        deformed_mesh = deform_mesh(mesh, self.decoder, encoding_expr, landmarks, lat_rep_shape=encoding_shape)
    ...
    def fit_val_dataset(self, epoch):
        self.decoder.eval()
        exp_dir = self.exp_path + 'val_recon/epoch_{}/'.format(epoch)
        ...
        for idx in range(len(self.val_dataset)):
            ...
            m_gt_pre = trimesh.load(os.path.join(val_dataset_path, iden_val, 'ori_sft_pre.obj'), process=False)
            
            m_gt_post = trimesh.load(os.path.join(val_dataset_path, iden_val, 'ori_sft_post.obj'), process=False)
            ...
            trim.export(exp_dir + f'{iden_val}_gt_pre.ply')
            m_gt_post.export(exp_dir + f'{iden_val}_gt_post.ply')
            trim_deformed.export(exp_dir + f'{iden_val}_pred_post_marching_cube.ply')

            trim_reg.export(exp_dir + f'{iden_val}_gt_pre_reg.ply')
            trim_deformed_reg.export(exp_dir + f'{iden_val}_pred_post_reg.ply')
